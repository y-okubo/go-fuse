...
for target in "clean" "install" ; do
  ...
  do
    ...
    go ${target} github.com/y-okubo/go-fuse/${d}
  done
done

for d in fuse zipfs unionfs
do
  (cd $d && go test github.com/y-okubo/go-fuse/$d && go test -race github.com/y-okubo/go-fuse/$d)
done

make -C benchmark
for d in benchmark
do
  go test github.com/y-okubo/go-fuse/benchmark -test.bench '.*' -test.cpu 1,2
done
